...
describe Sunspot::ActiveRecord::Configuration, "default values" do
  before(:all) do
    Sunspot::ActiveRecord.environment = "test"
    Sunspot::ActiveRecord.config = File.dirname(__FILE__) + "/sunspot.yml"
  end
  
  before(:each) do
    File.should_receive(:exist?).at_least(:once).and_return(false)
    @config = Sunspot::ActiveRecord::Configuration.new
  end
  ...
  it "should handle the 'solr_home' property when not set" do
    Sunspot::ActiveRecord.should_receive(:root).at_least(1).and_return('/some/path')
...
describe Sunspot::ActiveRecord::Configuration, "user settings" do
  before(:each) do
    Sunspot::ActiveRecord.stub!(:environment => 'config_test')
    Sunspot::ActiveRecord.config = File.dirname(__FILE__) + "/sunspot.yml"
