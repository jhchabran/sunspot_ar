module Sunspot #:nodoc:
  module ActiveRecord #:nodoc:
    # This module is simply Sunspot::Rails renamed to keep a coherent name. 
    # 
    # Sunspot::ActiveRecord is configured via the config/sunspot.yml file, which
    ...
    def self.config
      @config || (self.root + "config/sunspot.yml")
    end
    ...
    class Configuration
      ...
      def data_path
        @data_path ||= user_configuration_from_key('solr', 'data_path') || File.join(Sunspot::ActiveRecord.root, 'solr', 'data', Sunspot::ActiveRecord.environment)
      end
      
      def pid_path
        @pids_path ||= user_configuration_from_key('solr', 'pid_path') || File.join(Sunspot::ActiveRecord.root, 'solr', 'pids', Sunspot::ActiveRecord.environment)
      end
      ...
      def solr_home
        @solr_home ||=
          if user_configuration_from_key('solr', 'solr_home')
            ...
            File.join(Sunspot::ActiveRecord.root, 'solr')
          end
      end
      ...
      def default_log_file_location
        File.join(Sunspot::ActiveRecord.root, 'log', "solr_" + Sunspot::ActiveRecord.environment + ".log")
      end
          ...
          begin
            ...
            if File.exist?(path)
              File.open(path) do |file|
                YAML.load(ERB.new(file.read).result)[Sunspot::ActiveRecord.environment.to_s]
              end
            else
              {}
            end
